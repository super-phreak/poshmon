use std::{fmt, collections::HashMap, sync::{Arc, Mutex, RwLock}, error::Error};

use poshmon_lib::engine::gen1::{BasePokemon, PokeType, MoveType, GameState, PokeMove};
...
pub(super) fn build_moves(moves_json: serde_json::Value, typedex: Typedex) -> Result<PokeMove, Box<dyn Error>> {
    return Ok(PokeMove {
        ...
        priority: moves_json["priority"].as_i64().ok_or_else(|| DataFieldNotFoundError).and_then(|x| x.try_into().map_err(|_| DataFieldNotFoundError.into()))?,
    });
}
